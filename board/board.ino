...
void rpc_processor(int request_id, const String &method, const String params[], int params_size) {
  if (method == "set_builtin_led") {
    if (params_size != 1) {
      rpc_server.send_error(request_id, -32602, "Invalid params", "params_size != 1");
      return;
    }
    ...
    rpc_server.send_result(request_id, status ? "OK: builtin LED is ON" : "OK: builtin LED is OFF");

  } else {
    rpc_server.send_error(request_id, -32601, "Method not found", method.c_str());
  }
}
